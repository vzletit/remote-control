...
const prntCommands: CommandsTable = {
  scrn: async () => {
    ...
    if (mPos.x < 100) { mPos.x = 100 }
    if (mPos.x > await screen.width() - 100) { mPos.x = await screen.width() - 100 }
    if (mPos.y < 100) { mPos.y = 100 }
    if (mPos.y > await screen.height() - 100) { mPos.y = await screen.height() - 100 }
